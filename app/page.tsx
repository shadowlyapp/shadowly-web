...
import { fetchTranslation } from "@/app/api/fetchTranslation";
...
function PlayerPage() {
  ...
  const handleVideoSelect = (id: string) => {
    console.log("🔗 Clicked recommended video:", id);
    setVideoId(id);
    setCaptions([]);
    setTranscript([]);
  ...
  type PopulatedPlaylist = PlaylistConfig & {
    videos: VideoItem[];
  };
  ...
  useEffect(() => {
    ...
    console.log("🎯 useEffect: loaded id from URL =", id); // for debugging
    setVideoId(id);
  }, [searchParams]);
    ...
    const fetchTranscript = async () => {
      ...
      try {
        ...
        if (Array.isArray(data.captions)) {
          ...
          const exists = data.captions.some((c: { languageCode: string; isAutoGenerated?: boolean }) => 
            c.languageCode === selectedCaptionLang
          );
          if (!exists) {
...
const config: PlaylistConfig[] = await configRes.json();
        ...
        const fullPlaylists = await Promise.all(
          config.map(async (pl) => {
            ...
            const videos = Array.isArray(data.videos) ? data.videos.map((v: any) => ({
              videoId: v.videoId,
              title: v.title,
    ...
    const fetchDemoTranslation = async () => {
      try {
        const res = await fetch(`${API_BASE_URL}/api/translate`, {
          ...
          body: JSON.stringify({ word: "hello", source: "en", target: "es" }),
        });
  
        if (!res.ok) throw new Error(`Translation failed with status ${res.status}`);
        const data = await res.json();
        console.log("🌍 Demo Translation:", data.translatedText);
      } catch (err) {
